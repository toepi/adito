...
import com.adito.networkplaces.vfs2.provider.smb.SmbFileProvider;
...
import com.adito.networkplaces.store.file.AditoFileProvider;
...
public class NetworkPlacePlugin extends DefaultPlugin {
    ...
    void initFileSystems() throws FileSystemException {
        addProvider(new JarFileProvider(), "jar");
        addProvider(new ZipFileProvider(), "zip");
        addProvider(new TarFileProvider(), "tar");
        addProvider(new TgzFileProvider(), "tgz");
        addProvider(new Tbz2FileProvider(), "tbz2");
        addProvider(new GzipFileProvider(), "gz");
        addProvider(new TemporaryFileProvider(), "tmp");
        addProvider(new Bzip2FileProvider(), "bzip2", "bz2");
        addProvider(new SftpFileProvider(), "sftp");
        addProvider(new SmbFileProvider(), "cifs");
        addProvider(new WebdavFileProvider(), "webdav");


        final VFSProviderManager mgr = VFSProviderManager.getInstance();
        mgr.registerProvider(new AditoFileProvider());
    ...
    private void addProvider(final FileProvider provider, final String... schemas)
            throws FileSystemException {
        final FileSystemManager manager = VFS.getManager();
        final Set<String> newSchemas = new HashSet<String>(Arrays.asList(schemas));
        newSchemas.removeAll(Arrays.asList(manager.getSchemes()));
        if (!newSchemas.isEmpty() && manager instanceof StandardFileSystemManager) {
            final StandardFileSystemManager sfsm = (StandardFileSystemManager) manager;
            sfsm.addProvider(newSchemas.toArray(new String[newSchemas.size()]), provider);
        }
    }
    ...
    private void removeFileSystems() {
        ...
        mgr.deregisterProvider(SFTPStore.SFTP_SCHEME);
