...
/**
 * Extension to Jettys {@link org.mortbay.http.JsseListener} that loads its SSL
 * keys from Aditos keystore using the password that has been configured in the
 * property database.
 */
public class CustomJsseListener extends JsseListener {

    private static final Log LOG = LogFactory.getLog(CustomJsseListener.class);
    ...
    public CustomJsseListener() {
        super();
    }

    public CustomJsseListener(InetAddrPort address) {
        super(address);
    }

    /**
     * Set the trust manager to use. This must be done <b>before</b> the socket
     * factory is initialised (i.e. when Jetty is started). If the method is not
     * called the default trust managers will be used.
     *
    ...
    protected SSLServerSocketFactory createFactory() throws Exception {
        ...
        KeyManager[] kma = new KeyManager[]{new CustomKeyManager()};
        TrustManager[] tma;
        if (trustManager == null) {
            ...
            System.arraycopy(tm.getTrustManagers(), 0, tma, 0, tm.getTrustManagers().length - 1);
